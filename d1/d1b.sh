...
while read -r line; do
	...
	# special case if already at zero 
	if [[ $dial_position -eq 0 ]]; then
		distance_to_zero=100
	fi

	# clicks if the distance is over 100
	((click_count += move_distance/100))
	# clicks for getting to or passing zero
	if [[ $((move_distance%100)) -ge $distance_to_zero ]]; then
		((click_count++))
	fi
	
	# update dial position
	((dial_position += move_sign*(move_distance%100)+100))
	((dial_position %= 100))
